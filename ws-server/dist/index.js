// @bun
// src/dashscope/asr.ts
var ASR_WS_URL = "wss://dashscope-intl.aliyuncs.com/api-ws/v1/realtime?model=qwen3-asr-flash-realtime";
function createAsrSession(callbacks) {
...
var systemPrompt = readFileSync(resolve(import.meta.dir, "../../../prompts/system-prompt.md"), "utf-8");
async function streamLlmResponse(transcript, conversationHistory, onChunk, onDone, onError, signal, bargeInPrefix) {
  ...
  try {
    const messages = [
      ...
      ...bargeInPrefix ? [{ role: "assistant", content: bargeInPrefix }] : [],
...
// src/dashscope/tts.ts
var TTS_WS_URL = "wss://dashscope-intl.aliyuncs.com/api-ws/v1/realtime?model=qwen3-tts-vc-realtime-2026-01-15";
function createTtsSession(callbacks) {
  const apiKey = process.env.DASHSCOPE_API_KEY;
...
// src/types.ts
function isValidBrowserMessage(msg) {
  ...
  websocket: {
    ...
    message(ws, raw) {
      ...
      if (!isValidBrowserMessage(parsed)) {
        session.send({ type: "error", message: `Unknown message type: ${parsed?.type ?? "unknown"}` });
        return;
      }
      switch (parsed.type) {
        case "session.start":
